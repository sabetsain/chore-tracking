...
export function ChoreDutyView({
  ...
  return (
    <div className="space-y-6 notebook-ruled-surface notebook-margin-guide p-4 sm:p-6 rounded-xl border border-slate-300 dark:border-slate-700">
      {/* Top Banner / Member Status */}
      <PaperCard variant="card" className="p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border border-slate-300 dark:border-slate-700">
        <div>
          <div className="flex items-center gap-2">
            <h2 className="text-2xl sm:text-3xl font-hand font-bold text-ink-navy dark:text-slate-100">Weekly Chore Duties</h2>
            {isAway && (
              <span className="px-2.5 py-0.5 rounded-lg bg-amber-100 dark:bg-amber-900/50 border border-amber-300 dark:border-amber-700 text-amber-900 dark:text-amber-200 text-xs font-hand font-bold">
                Away
              </span>
            )}
          </div>
          <p className="text-xs text-ink-graphite dark:text-slate-400 mt-1 font-body">
            {isAway
        ...
        <button
          ...
          className={`inline-flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-hand font-bold transition border shadow-paper-sm active:scale-95 ${
            isAway
              ? 'bg-amber-600 hover:bg-amber-700 text-white border-amber-700'
              : 'bg-paper-card dark:bg-[#283548] hover:bg-amber-100/60 dark:hover:bg-slate-700 text-ink-navy dark:text-slate-200 border-slate-300 dark:border-slate-600'
          }`}
        >
          {isAway ? (
            <>
      ...
      <div className="space-y-4 pt-2">
        <div className="flex items-center justify-between pb-1 border-b border-slate-200/80 dark:border-slate-700/80">
          ...
          <span className="font-hand font-bold text-sm text-ink-graphite dark:text-slate-400 bg-amber-100/60 dark:bg-slate-700 px-2 py-0.5 rounded border border-amber-200/60 dark:border-slate-600">
            {myAssignments.filter((a) => a.status === 'completed').length} / {myAssignments.length} done
          </span>
        </div>

        {myAssignments.length === 0 ? (
          <PaperCard variant="card" className="p-6 text-center text-sm font-hand text-ink-muted dark:text-slate-400 border-dashed border-2 border-slate-300 dark:border-slate-700">
              ...
              >
                <div>
                  <div className="flex items-start justify-between gap-2 mb-2">
                    <div className="flex items-center gap-2">
                      <ScribbleCheckbox
                        ...
                        label={
                          <span className="font-hand font-bold text-lg text-ink-navy dark:text-slate-100 leading-snug">
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded bg-highlighter-yellow text-amber-950 font-hand font-bold text-xs border border-amber-300/80 shadow-sm">
                  ...
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    {assignment.status === 'completed' && (
                      <span className="text-[11px] font-hand font-bold px-2 py-0.5 rounded bg-emerald-100 dark:bg-emerald-900/50 text-stamp-clean dark:text-emerald-300 border border-emerald-300 dark:border-emerald-700">
                        Completed
                      </span>
                    )}
                  </div>
                  ...
                  {assignment.chore.completion_type === 'continuous_duty' && (
                    <div className="my-2 p-2 bg-paper-sheet dark:bg-[#1e293b] rounded border border-slate-200 dark:border-slate-700">
                      <TallyCounter
                ...
                {/* Actions */}
                <div className="flex items-center gap-2 pt-3 mt-2 border-t border-slate-200/80 dark:border-slate-700/80">
                  {assignment.chore.completion_type === 'single_weekly' ? (
                    assignment.status === 'completed' ? (
                      <div className="flex-1 py-2 text-center font-hand font-bold text-sm text-stamp-clean bg-emerald-100/60 dark:bg-emerald-950/40 rounded-lg border border-emerald-300 dark:border-emerald-700">
                        Done for this week
                      </div>
                    ) : (
                      <button
                        ...
                        className="flex-1 py-2 px-3 bg-indigo-700 hover:bg-indigo-800 text-white font-hand font-bold text-sm rounded-lg shadow-paper-sm transition flex items-center justify-center gap-1.5 disabled:opacity-50 active:scale-95"
                      >
                    ...
                    <button
                      ...
                      className="flex-1 py-2 px-3 bg-indigo-700 hover:bg-indigo-800 text-white font-hand font-bold text-sm rounded-lg shadow-paper-sm transition flex items-center justify-center gap-1.5 active:scale-95 border-t border-dashed border-indigo-400"
                    >
                  ...
                  {onOpenSwap && assignment.status === 'pending' && (
                    <button
                      ...
                      className="py-2 px-3 bg-paper-manila dark:bg-[#334155] hover:bg-amber-200/80 dark:hover:bg-slate-600 text-ink-navy dark:text-slate-200 font-hand font-bold text-sm rounded-lg border-2 border-dashed border-amber-300 dark:border-slate-500 shadow-paper-sm transition flex items-center gap-1 active:scale-95"
      ...
      {Array.from(otherMembersMap.values()).map(({ member, assignments: memberAssignments }) => (
        <div key={member.id} className="space-y-4 pt-4 border-t border-slate-200/80 dark:border-slate-700/80">
          <div className="flex items-center justify-between pb-1">
            <h3 className="text-xl font-hand font-bold text-ink-navy dark:text-slate-100 flex items-center gap-2">
              <User className="w-4 h-4 text-ink-graphite dark:text-slate-400" />
              <span>{member.nickname}'s Duties</span>
              {member.status === 'away' && (
                <span className="px-2 py-0.5 rounded text-xs font-hand font-bold uppercase bg-amber-100 dark:bg-amber-900/50 text-amber-900 dark:text-amber-200 border border-amber-300 dark:border-amber-700">
                  Away
                </span>
              )}
            </h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {memberAssignments.map((assignment, idx) => (
              <PaperCard
                ...
                className="p-4 flex flex-col justify-between border border-slate-300 dark:border-slate-700"
              >
                <div>
                  <div className="flex items-start justify-between gap-2 mb-1.5">
                    <h4 className="font-hand font-bold text-lg text-ink-navy dark:text-slate-100 leading-snug">
                      {assignment.chore.title}
                    </h4>
                  ...
                  {assignment.chore.description && (
                    <p className="text-xs text-ink-graphite dark:text-slate-400 mb-2 font-body">
                      {assignment.chore.description}
                    </p>
                  )}
                ...
                {assignment.chore.completion_type === 'continuous_duty' && (
                  <div className="pt-3 mt-3 border-t border-slate-200/80 dark:border-slate-700/80">
                    <button
                      type="button"
                      onClick={() => setActiveLogAssignment(assignment)}
                      className="w-full py-1.5 px-2.5 bg-paper-card dark:bg-[#283548] hover:bg-amber-100/60 dark:hover:bg-slate-700 text-ink-navy dark:text-slate-200 font-hand font-bold text-xs rounded-lg border border-slate-300 dark:border-slate-600 transition flex items-center justify-center gap-1.5 shadow-paper-sm active:scale-95"
                    >
