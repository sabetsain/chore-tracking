...
export function PWAInstallPrompt({ onDismiss }: PWAInstallPromptProps) {
  ...
  return (
    <aside
      ...
      className="bg-indigo-900 text-white rounded-2xl p-4 shadow-lg border border-indigo-700/50 mb-6 relative overflow-hidden"
    >
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-3">
          <div className="p-2.5 bg-indigo-800 rounded-xl shrink-0 text-indigo-200">
        ...
        <button
          ...
          className="text-indigo-300 hover:text-white p-1 rounded-lg hover:bg-indigo-800 transition"
        >
          <X className="w-4 h-4" />
        </button>
      </div>
      ...
      {deferredPrompt && (
        <div className="mt-3 flex items-center justify-end gap-2">
          <button
            type="button"
            onClick={handleDismiss}
            className="px-3 py-1.5 text-xs text-indigo-200 hover:text-white font-medium transition"
          >
            Dismiss
          </button>
          <button
            ...
            className="inline-flex items-center gap-1.5 px-3.5 py-1.5 bg-indigo-500 hover:bg-indigo-400 text-white rounded-xl text-xs font-semibold shadow transition"
          >
      ...
      {!deferredPrompt && isIOS && (
        <div className="mt-3 pt-3 border-t border-indigo-800/80 text-xs text-indigo-100 space-y-1.5">
