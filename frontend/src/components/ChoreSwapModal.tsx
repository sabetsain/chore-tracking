      ...
      <div className="relative max-w-md w-full">
        ...
        >
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {/* Source chore */}
            <div>
              <label className="block text-xs font-bold text-ink-graphite dark:text-slate-400 uppercase tracking-wider mb-1 font-body">
                Your Current Chore
              </label>
              <div className="p-3.5 bg-paper-sheet dark:bg-[#1e293b] border border-amber-300/70 dark:border-slate-700 rounded-lg shadow-paper-sm">
                <div className="flex items-center justify-between">
                  <span className="font-hand font-bold text-lg text-ink-navy dark:text-slate-100">
                    {sourceAssignment.chore.title}
                  </span>
                  <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded bg-highlighter-yellow text-amber-950 text-xs font-hand font-bold border border-amber-300/80 shadow-sm">
            ...
            <div>
              <label className="block text-xs font-bold text-ink-graphite dark:text-slate-400 uppercase tracking-wider mb-2 font-body">
                Select Roommate's Chore to Swap With
              </label>

              {availableTargets.length === 0 ? (
                <p className="text-xs font-hand text-ink-muted dark:text-slate-400 p-4 bg-paper-sheet dark:bg-[#1e293b] rounded-lg text-center border border-dashed border-slate-300 dark:border-slate-700">
                    ...
                    return (
                      <div
                        ...
                        className={`p-3 rounded-lg border cursor-pointer transition flex items-center justify-between shadow-paper-sm ${
                          isSelected
                            ? 'border-indigo-600 bg-paper-sheet dark:bg-[#1e293b] ring-2 ring-indigo-500/40'
                            : 'border-amber-200 dark:border-slate-700 hover:border-amber-400 bg-paper-sheet/80 dark:bg-[#1e293b]/80'
                        }`}
                      >
                        <div>
                          <div className="font-hand font-bold text-base text-ink-navy dark:text-slate-100">
                            {target.chore.title}
                          </div>
                          <div className="text-xs text-ink-graphite dark:text-slate-400 mt-0.5 font-body">
                            Assigned to:{' '}
                            <span className="font-hand font-bold text-ink-navy dark:text-slate-200 text-sm">
                        ...
                        <div className="flex items-center gap-2">
                          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded bg-highlighter-yellow text-amber-950 text-xs font-hand font-bold border border-amber-300/80">
            ...
            <div className="flex gap-2 pt-3 border-t border-amber-300/60 dark:border-slate-600">
              <button
                type="button"
                onClick={onClose}
                className="flex-1 py-2.5 bg-paper-card dark:bg-[#283548] hover:bg-amber-100/60 dark:hover:bg-slate-700 text-ink-graphite dark:text-slate-300 text-sm font-hand font-bold rounded-lg border border-slate-300 dark:border-slate-600 transition"
              >
                Cancel
              </button>
              <button
