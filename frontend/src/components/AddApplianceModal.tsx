    ...
    <AnimatePresence>
      ...
      >
        ...
        >
          <PaperCard
            variant="card"
            className="p-6 sm:p-7 shadow-paper-lifted bg-canvas-card border border-stone-300 dark:border-slate-700 max-h-[90vh] flex flex-col"
          >
            {/* Header */}
            ...
            {error && (
              <div className="mb-4 p-3 rounded-lg bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 text-accent-crimson dark:text-red-300 text-xs flex items-center gap-2 font-sans font-medium">
            ...
            <form onSubmit={handleSubmit} className="space-y-5 overflow-y-auto pr-1">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {PRESET_TEMPLATES.map((tpl) => (
                      <button
                        ...
                        className="px-2.5 py-1 text-xs font-sans rounded-md bg-stone-100 dark:bg-slate-800 text-ink-graphite dark:text-slate-300 hover:bg-stone-200 dark:hover:bg-slate-700 border border-stone-200 dark:border-slate-700 transition active:translate-y-[1px]"
                      >
                        {tpl.name}
                      </button>
                    ))}
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-3.5 py-2 bg-canvas-card dark:bg-[#222D42] border border-stone-300 dark:border-slate-600 rounded-lg text-sm text-ink-navy dark:text-slate-100 placeholder:text-ink-muted dark:placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-accent-slate transition font-sans"
                />
              </div>
                  ...
                  {ICON_OPTIONS.map((opt) => {
                    ...
                    return (
                      <button
                        ...
                        className={`flex flex-col items-center justify-center p-2 rounded-lg border transition text-center active:translate-y-[1px] ${
                          isSelected
                            ? 'bg-[#EAEFF5] dark:bg-[#1E293B] border-accent-slate text-accent-slate dark:text-slate-200 font-bold shadow-sm'
                            : 'bg-canvas-card dark:bg-[#222D42] border-stone-200 dark:border-slate-700 text-ink-graphite dark:text-slate-300 hover:border-accent-slate'
                        }`}
                        title={opt.label}
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  {steps.length < 5 && (
                    <button
                      ...
                      className="inline-flex items-center gap-1 text-xs font-semibold text-accent-slate dark:text-slate-300 hover:underline"
                    >
                ...
                <div className="space-y-2 bg-stone-50/70 dark:bg-slate-800/40 p-3 rounded-lg border border-stone-200 dark:border-slate-700">
                  {steps.map((step, idx) => (
                    <div
                      key={idx}
                      className="flex items-center gap-2 bg-canvas-card dark:bg-[#1A2234] p-2 rounded-md border border-stone-200 dark:border-slate-700 shadow-paper-sm"
                    >
                      <span className="w-6 h-6 rounded-full bg-stone-200 dark:bg-slate-700 text-accent-slate dark:text-slate-200 font-mono text-xs font-bold flex items-center justify-center shrink-0">
                        {idx + 1}
                      </span>
                      ...
                      <select
                        ...
                        className="flex-1 px-2.5 py-1.5 bg-canvas-card dark:bg-[#222D42] border border-stone-300 dark:border-slate-600 rounded text-xs font-sans font-medium text-ink-navy dark:text-slate-100 focus:outline-none focus:ring-1 focus:ring-accent-slate"
                      >
                      ...
                      <div className="flex items-center gap-0.5 shrink-0">
                        ...
                        {steps.length > 2 && (
                          <button
                            ...
                            className="p-1 rounded text-accent-crimson hover:text-red-700 transition"
                            title="Remove step"
              ...
              <div className="pt-2 border-t border-stone-200 dark:border-slate-700 space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <Clock className="w-4 h-4 text-accent-slate dark:text-slate-400" />
                    <div>
                  ...
                  <input
                    ...
                    className="w-4 h-4 rounded text-accent-slate focus:ring-accent-slate"
                ...
                {timerEnabled && (
                  <div>
                    ...
                    <input
                      ...
                      className="w-32 px-3 py-1.5 bg-canvas-card dark:bg-[#222D42] border border-stone-300 dark:border-slate-600 rounded text-sm text-ink-navy dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-accent-slate font-mono tabular-nums"
                    />
                  </div>
                )}
              </div>
              ...
              <div className="flex gap-2 pt-3 border-t border-stone-200 dark:border-slate-700">
                <button
                  type="button"
                  onClick={onClose}
                  className="flex-1 py-2.5 bg-canvas-card dark:bg-[#222D42] hover:bg-stone-100 dark:hover:bg-slate-700 text-ink-graphite dark:text-slate-300 text-sm font-sans font-semibold rounded-lg border border-stone-300 dark:border-slate-600 transition active:translate-y-[1px]"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  disabled={submitting}
                  className="flex-1 py-2.5 bg-accent-slate hover:bg-[#1E334A] text-white text-sm font-sans font-bold rounded-lg shadow-paper-sm transition active:translate-y-[1px] disabled:opacity-50 flex items-center justify-center gap-2"
                >
