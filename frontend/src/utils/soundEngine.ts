export class StationerySoundEngine {
  ...
  /**
   * Subtle Acoustic Chime Synthesis.
   * Produces a warm, resonant dual-harmonic acoustic chime for timer completion alerts.
   */
  public playChimeSound(velocity: number = 1.0): void {
    ...
    try {
      ...
      // Fundamental harmonic (587.33 Hz - D5)
      const osc1 = ctx.createOscillator();
      const gain1 = ctx.createGain();
      osc1.type = 'sine';
      osc1.frequency.setValueAtTime(587.33, now);

      gain1.gain.setValueAtTime(0.25 * safeVelocity, now);
      ...
      // Overtone harmonic (880 Hz - A5)
      const osc2 = ctx.createOscillator();
      const gain2 = ctx.createGain();
      osc2.type = 'sine';
      osc2.frequency.setValueAtTime(880, now + 0.04);

      gain2.gain.setValueAtTime(0.18 * safeVelocity, now + 0.04);
